...
export class SociasListComponent implements OnInit, OnDestroy {
  socias: Socia[] = [];
  loading = false;
  error = '';
...
export class SociasListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.sociaService.getAll().subscribe({
      next: (data) => this.socias = data,
      error: (err) => console.error('Error al cargar socias:', err)
    });
  }
  ...
  eliminarSocia(index: number) {
    if (confirm('¿Estás seguro de eliminar esta socia?')) {
      // Aquí puedes llamar al servicio si deseas que también se elimine del backend
      const id = this.socias[index].id;
      this.sociaService.delete(id).subscribe({
        next: () => this.socias.splice(index, 1),
        error: (err) => console.error('Error al eliminar socia:', err)
      });
    }
  }
}*/